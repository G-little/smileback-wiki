# 自定义消息格式文档

[TOC]
## 修订记录
----
日期 | 作者 | 修订类型 | 修订内容 | 版本|
---- | ---- | ---- | ---- | ---- |
2020年07月10日|冷立纲|A|新增设计方案|1.0|

> 【修订类型：A-新增  M-修改 D-删除】

## 背景

自定义消息格式文档

## 产品说明

满足产品自定义展示消息

## 关键账户说明


| 用户ID | 身份  |
|:----:|:---:|
| 9999 | 通知 |
| 9998 | 用户通知 |
| 9997 | 官方小二 |



## 接口说明

自定义协议格式说明

``` json

		{
    "SyncOtherMachine": 1, //消息同步至发送方
    "From_Account": "10001",
    "To_Account": "10002",
    "MsgRandom": 1287657,
    "MsgTimeStamp": 5454457,
    "MsgBody": [
        {
            "MsgType": "TIMCustomElem", //自定义消息类型
            "MsgContent": {    
				     "type": 1,  //协议号 依次递增  具体格式参见下面协议定义
				     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别		  
				     "tips":"消息会话列表显示提示",
				     "createTime": 1233434343l, //创建时间戳
				     "msg":{                    //消息体
				      "title":"标题", //标题
				      "url":"http://kistalk.com", // h5 广告页面
				      "content":"这是消息内容",
				      "imageUrl":"头图地址", //相对地址，自行追加
				      "coverWidth":300,  //高度
				      "coverHeight":400   //宽度
				      }
            }
        }
    ]
}
		
```




### 1) 文本消息

#### 接口说明

| Type          | 1             |
|:------------- |:-------------:|
| 版本号      |  1.0 |



####  协议说明


```json

      {    
			     "type": 1,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			         "text":'xxxx'
			      }
        }


```


### 2) 匹配结果通知

#### 接口说明

| Type          | 2            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 9999 |



####  协议说明


```json

      {    
			     "type": 2,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			         "uid": 12345,
			         "avatar": "/xxx", //头像
			         "roomId": "xxxxxx" //房间号
			      }
        }


```


### 3)  好友匹配成功消息

#### 接口说明

| Type          | 3            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 主动加好友的用户ID |



####  协议说明


```json

      {    
			     "type": 3,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			         "likePositions":[
			             {
			                 "uid":123,//用户ID
			                 "roomId":123,//房间ID
			                 "roomName":"xxx",//房间名
			                 "imageUrl":"xxx",//房间照片  
			             }
			         ]
			      }
        }


```


### 4)  评论消息

#### 接口说明

| Type          | 4            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 9998 |



####  协议说明


```json

      {    
			     "type": 4,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			         "avatar": "/xxx", //头像
			         "uid":123, //用户ID
			         "remark": ,//备注
			         "comment": "描述", //用户ID
			         "roomId": "xxx", //房间ID
			         "roomName": "xxx", //房间名
			         "noteId": 123, //留言ID
			         "note": 123 //留言内容
			         "commentId": 123 //留言ID
			      }
        }


```


### 5)  头像审核消息

#### 接口说明

| Type          | 5            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 9999 |



####  协议说明


```json

      {    
			     "type": 5,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			         "avatar": "/xxx", //头像
			         "status":-1 , // -1 驳回 1 通过
			         "comment": " xxx" // 描述
			      }
        }


```


### 6)  弹幕消息

#### 接口说明

| Type          | 6            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 9999 |



####  协议说明


```json

      {    
			     "type": 6,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			         "content": " xxx", // 内容
			         "userRole":0, //用户角色  0:普通 1:boss 2:员工
			      }
        }


```


### 7)  留言通知

#### 接口说明

| Type          | 7            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 9999 |



####  协议说明


```json

      {    
			     "type": 7,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			         "action": " ADD", // 行为  ADD  UPDATE DELETE
			         "id":1, //留言Id
			         "roomId":"xxxxx", //房间ID
			         "commentCount":"123", //评论计数
			         "note":{
			                "interactType": 1 ,//1:点赞，默认:评论 
                        "avatar": "/x/y/z",   //留言数据
                        "uid": 11443,
                        "id": 2,
                        "roomId": "B0FFLIND01", //空间ID
                        "note": "test",
                        "color": "#fffff",  //颜色
                        "addTime": 1594198044901,
                        "commentsCount": 10,
                        "noteType": 1, // 0 文本 1 图片
                        "imageUrl": "/xxxx"   
                    }
			         
			      }
        }


```


### 8)  房间用户变更通知

#### 接口说明

| Type          | 8            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 9999 |



####  协议说明


```json

      {    
			     "type": 8,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			         "action": "QUIT", // 行为  QUIT JOIN
			         "roomId":1, //留言Id
			         "uid":"123", //房间ID
			         "avatar":"/xxxx", //评论计数
			         "userRole":1,
			         "userCount":1
			      }
        }


```



### 9) 图片消息  

#### 接口说明

| Type          | 9            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 用户 |



####  协议说明


```json

      {    
			     "type": 9,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			         "url":"/a",
			         "w":333,  //宽
			         "h":444 //高
			      }
        }


```



### 1001 ) 视频消息  

#### 接口说明

| Type          | 1001            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 用户 |



####  协议说明


```json

      {    
			     "type": 1001,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			         "videoUrl":"/a",
			         "coverUrl":"/b",
			         "length":15, //视频时长
			         "w":333,  //宽
			         "h":444 //高
			      }
        }


```



### 11) 产品消息  

#### 接口说明

| Type          | 11            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 用户 |



####  协议说明


```json

      {    
			     "type": 11,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{                    //文本消息
			             "id": 10000053,
			             "goodsName": "面",
			             "picUrl": "Fs-UctrCkT-ja4UCulFAfTGIa7V2.jpg",
			             "price": 0.02,
			             "storeInfo": {
			                 "id": 24,
			                 "name": "真传泰拳馆",
			                 "intro": null,
			                 "roomId": "B0G39HMP4L",
			                 "images": [
                            "http://resources.kinstalk.com/yaljtdqiezt1l4cs5z8s.jpg"
                            ],
                        "owner": 12748
                        }
                }
        }


```


### 12) 服务通知

#### 接口说明

| Type          | 12            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 9996 |



####  协议说明


```json

      {    
			     "type": 12,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{
			         "role":1 , //1用户  2 店铺的
			         "noticeType":  1, //通知类型   1 通用  2 订单  3 售后 4 物流  5 退货物流（）      
			         "title":"待发货通知",
			         "content":"通知内容",
			         "imageUrl":"/xxxx",
			         "ext": {
			             "oderId":123,
			             "aftersaleId":123,
			             "storeId":2222, //店铺ID
			         }    //附加信息
			     }
        }


```



### 13) 直播状态通知

#### 接口说明

| Type          | 13            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 群消息 |



####  协议说明


```json

      {    
			     "type": 13,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{
			        "action":"START" , // START / STOP
			        "liveType": "VIDEO" , // VIDEO  VOICE
			        "playUrl":"rtmp://xxx" //播放地址
			        
			     }
        }


```



### 15) 陪伴灯协议

#### 接口说明

| Type          | 15            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 群消息 |



####  协议说明


```json

      {    
			     "type": 13,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{
			        "light": true , //点灯状态
			        "lightTime": 122333 // 点灯开始时间毫秒
			        
			     }
        }


```


### 16) 视频变更通知

#### 接口说明

| Type          | 16            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 群消息 |



####  协议说明


```json

      {    
			     "type": 16,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{
			        "roomId": "xxxxxx" , //空间ID
			        "action": "ADD" // 添加或删除
			        
			     }
        }


```


### 17) 直播状态变更通知

#### 接口说明

| Type          | 17            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 群消息 |



####  协议说明


```json

      {    
			     "type": 17,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{
			        "talkId": "xxxxxx" , //直播ID
			        "roomId": "xxxxxx" , //空间ID
			        "status": 0, // 直播状态 0 预约 1 开始 -1 结束 -2 取消 
			        "streamId":"xxxx", //推流id
			        "uid":123 //用户ID  
			     }
        }


```



### 18) 举手状态变更通知

#### 接口说明

| Type          | 18            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 群消息 |



####  协议说明


```json

      {    
			     "type": 18,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{
			        "talkId": "xxxxxx" , //直播ID
			        "handStatus": 0, // 举手状态 0 放下 1 举手
			        "uid":123 //用户ID  
			     }
        }


```



### 19) 提问通知

#### 接口说明

| Type          | 19            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 9999 |



####  协议说明


```json

      {    
			     "type": 19,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{
			        "roomId": "xxxxxx" , //空间ID
			        "talkId": 0, // 沙龙ID
			        "uid":123, //发送人ID  
			        "questionId":123, //提问Id
			        "question":"提问" //提问内容
			     }
        }


```


### 20) 移除用户通知

#### 接口说明

| Type          | 20            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 9999 |



####  协议说明


```json

      {    
			     "type": 20,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{
			        "roomId": "xxxxxx" , //空间ID
			        "roomName": "空间名称" // 空间名称			     }
        }


```



### 21) 对讲状态变更通知

#### 接口说明

| Type          | 21            |
|:------------- |:-------------:|
| 版本号      |  1.0 |
| 来源账号      | 9999 |



####  协议说明


```json

      {    
			     "type": 21,  //协议号 依次递增  具体格式参见下面协议定义
			     "version":"1.0", //协议版本号，本地配置目前能处理的版本号，高于该版本则提示需要升级识别	  
			     "tips":"消息会话列表显示提示",
			     "createTime": 1233434343l, //创建时间戳
			     "msg":{
			        "talk": {
                        "id": 1,
                        "roomId": "InCPRnDIdsekVcR",//空间ID
                        "streamId": null,
                        "mayStartTime": 1615395600000, //预约时间
                        "uid": 11455,//用户ID
                        "status": -1, //沙龙状态 0: 预约 -1 结束 -2 删除 1 直播中
                        "content": "11", // 沙龙内容
                        "subscribed": false,  // 是否已预约
                        "subscribeCount": null, // 预约人数
                        "inviteType": null,
                        "handsCount": 0,
                        "addTime": 1615356361114, //发起时间
                        "startTime": null //实际开始时间
                    }
             }
        }


```